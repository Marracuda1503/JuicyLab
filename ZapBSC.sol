...
contract ZapBSC is OwnableUpgradeable {
    ...
    function zapInToken(address _from, uint amount, address _to) external {
        ...
        if (isFlip(_to)) {
            ...
            if (_from == token0 || _from == token1) {
                ...
                if (_from == WBNB) {
                    _swapWBNBToFlip(_to, amount, msg.sender);
                } else {
                    uint bnbAmount = _swapTokenForBNB(_from, amount, address(this));
    ...
    function _swapWBNBToFlip(address flip, uint amount, address receiver) private {
        require(isFlip(flip), "ZapBSC: non-flip");
        ...
        if (token0 == WBNB || token1 == WBNB) {
            ...
            uint tokenAmount = _swapWBNBForToken(token, swapValue, address(this));

            _approveTokenIfNeeded(token);
            ROUTER.addLiquidityETH{value : amount.sub(swapValue)}(token, tokenAmount, 0, 0, receiver, block.timestamp);
        } else {
            uint swapValue = amount.div(2);
            uint token0Amount = _swapWBNBForToken(token0, swapValue, address(this));
            uint token1Amount = _swapWBNBForToken(token1, amount.sub(swapValue), address(this));
    ...
    function _swapWBNBForToken(address token, uint value, address receiver) private returns (uint) {
        address[] memory path;
        ...
        uint[] memory amounts = ROUTER.swapExactTokensForTokens(value, 0, path, receiver, block.timestamp);
